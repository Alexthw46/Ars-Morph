...
public class WildenHunterAbility extends MorphBoundAbility<WildenHunter> {

    static final Spell summon = new Spell(EffectSummonWolves.INSTANCE, AugmentExtendTime.INSTANCE);

    public WildenHunterAbility() {
        super(WildenHunter.class);
    }

    @Override
    protected void use(Player player, WildenHunter wildenHunter) {
        ...
        if (!resolver.postEvent().isCanceled()) {
            ...
            player.removeEffect(ModPotions.SUMMONING_SICKNESS_EFFECT);
        }
    }
